...
class RegistrationRepository extends Repository {
  ...
  insertAccount = (user: User): Promise<number> => {
    const sql = `INSERT INTO account (username, sha_pass_hash, email, reg_mail)
                     VALUES (?, ?, ?, ?)`
    return this.pool
      .query(sql, [user.username, user.password, user.email, user.email])
      .then(
        ([r]: [OkPacket]) => {
          return r.insertId
        },
  ...
  insertUser = (user: User, idAccount: number): Promise<number> => {
    const sql = `INSERT INTO user (nickname, id_account)
                     VALUES (?, ?)`
    return this.pool.query(sql, [user.nickname, idAccount]).then(
      ([r]: [OkPacket]) => {
        return r.insertId
      },
      (err: MysqlError) => {
  ...
  updateSecure = (user: User): Promise<number> => {
    const sql = `update account a
            join user u on a.id = u.id_account
                     set a.email = ?
                     where u.id = ?`
    return this.pool.query(sql, [user.email, user.id]).then(
      ([r]: [OkPacket]) => {
        if (!r.affectedRows) {
  ...
  insertAccountEmail = (user: User): Promise<number> => {
    const sql = `INSERT INTO user_email (id_user, email, token)
                     VALUES (?, ?, ?)`
    return this.pool.query(sql, [user.id, user.email, user.token]).then(
      ([r]: [OkPacket]) => {
        return r.insertId
      },
      (err: MysqlError) => {
  ...
  updatePassword = (user: UserPassword): Promise<number> => {
    const sql = `update account a
            join user u on a.id = u.id_account
                     set a.sha_pass_hash = ?
                     where u.id = ?`
    return this.pool.query(sql, [user.password, user.id]).then(
      ([r]: [OkPacket]) => {
        if (!r.affectedRows) {
          throw new NotFoundError('Не найден пользователь')
        }
