...
export class Smtp {
  ...
  constructor(private configProvider: ConfigProvider) {
    this.transport = createTransport(
      configProvider.get<SMTPTransport.Options>('smtp')
    )
  }
  ...
  public sendConfirmation = (
    ...
    const mailOptions = {
      from: `Equilibrium: RP сервере WOW robot <${this.configProvider.get(
        'smtp.auth.user'
      )}>`,
      to: email,
  ...
  private htmlConfirmation = (nickname: string, token: string): string => {
    const link = `http://${this.configProvider.get('host')}${
      this.configProvider.get('port') !== 80
        ? ':' + this.configProvider.get('port')
        : ''
    }/login?token=${token}`
