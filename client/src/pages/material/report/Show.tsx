...
class ReportPage extends React.Component<P, S> {
  ...
  componentDidUpdate(prevProps: Readonly<P>, prevState: Readonly<S>) {
    ...
    if (!this.state.isAdmin && this.context.user.id > 0) {
      const isAdmin =
        this.state.report.coauthors.findIndex((el: User) => {
          return el.id === this.context.user.id
        }) === -1
          ? this.context.user.id === this.state.report.idUser
          : true
      if (isAdmin) {
        this.setState({
          isAdmin,
    ...
    return (
      <Page>
        ...
        <ConfirmationWindow
          ...
          title="Вы действительно хотите удалить отчет / лог?"
        />
        ...
        <InfoBlock title="Описание отчета / лога">
          {this.state.report.description}
        </InfoBlock>
        <InfoBlock title="Важная информация">
          {this.state.report.rule}
        </InfoBlock>
        <Card
          title="Участники"
          href="/material/character/"
          list={this.state.report.members}
        />
        <List
          title="Гильдии"
          href="/material/guild/"
          list={this.state.report.guilds}
        />
        <List
          title="Сюжеты"
          href="/material/story/"
          list={this.state.report.stores}
        />
        ...
        <About
          author={this.state.report.userNickname}
          coauthors={this.state.report.coauthors}
