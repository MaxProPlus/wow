...
class StoryPage extends React.Component<P, S> {
  ...
  componentDidUpdate(prevProps: Readonly<P>, prevState: Readonly<S>) {
    ...
    if (!this.state.isAdmin && this.context.user.id > 0) {
      const isAdmin =
        this.state.story.coauthors.findIndex((el: User) => {
          return el.id === this.context.user.id
        }) === -1
          ? this.context.user.id === this.state.story.idUser
          : true
      if (isAdmin) {
        this.setState({
          isAdmin,
    ...
    return (
      <Page>
        ...
        <ConfirmationWindow
          ...
          title="Вы действительно хотите удалить сюжет?"
        />
        ...
        <InfoBlock title="Дополнительные сведения">
          {this.state.story.more}
        </InfoBlock>
        <Card
          title="Участники"
          href="/material/character/"
          list={this.state.story.members}
        />
        <List
          title="Гильдии"
          href="/material/guild/"
          list={this.state.story.guilds}
        />
        <List
          title="Отчеты"
          href="/material/report/"
          list={this.state.story.reports}
        />
        ...
        <About
          author={this.state.story.userNickname}
          coauthors={this.state.story.coauthors}
