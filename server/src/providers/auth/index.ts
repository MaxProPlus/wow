...
class AuthProvider {
    ...
    login = async (user: User, about: About): Promise<string> => {
        ...
        if (!lUser.id) {
            ...
            lUser.id = await this.registrationRepository.insertUser(user, lUser.idAccount)
        }
    ...
    getUserByTokenAndPassword = async (token: string, password: string): Promise<User | null> => {
        if (!token || !password) {
            return null
        }
        ...
        const user = await this.authRepository.getUserByTokenAndPassword(token, password)
        if (!user) {
            return null
        }
        return user
