...
class GuildPage extends React.Component<P, S> {
  ...
  componentDidUpdate(prevProps: Readonly<P>, prevState: Readonly<S>) {
    ...
    if (!this.state.isAdmin && this.context.user.id > 0) {
      const isAdmin =
        this.state.guild.coauthors.findIndex((el: User) => {
          return el.id === this.context.user.id
        }) === -1
          ? this.context.user.id === this.state.guild.idUser
          : true
      if (isAdmin) {
        this.setState({
          isAdmin,
    ...
    return (
      <Page>
        ...
        <ConfirmationWindow
          ...
          title="Вы действительно хотите удалить гильдию?"
        />
        ...
        <InfoBlock title="Описание и история">
        ...
        <Card
          ...
          list={this.state.guild.members}
        />
        <List
          title="Сюжеты"
          href="/material/story/"
          list={this.state.guild.stores}
        />
        <List
          title="Отчеты"
          href="/material/report/"
          list={this.state.guild.reports}
        />
        ...
        <About
          author={this.state.guild.userNickname}
          coauthors={this.state.guild.coauthors}
