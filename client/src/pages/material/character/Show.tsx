...
class CharacterPage extends React.Component<P, S> {
  ...
  componentDidUpdate(prevProps: Readonly<P>, prevState: Readonly<S>) {
    ...
    if (!this.state.isAdmin && this.context.user.id > 0) {
      const isAdmin =
        this.state.character.coauthors.findIndex((el: User) => {
          return el.id === this.context.user.id
        }) === -1
          ? this.context.user.id === this.state.character.idUser
          : true
      if (isAdmin) {
        this.setState({
          isAdmin,
        ...
        <div className="page-character">
          <PageTitle title="Персонаж" icon={icon}>
            <ControlButton
              ...
              id={this.state.id}
              href="/material/character"
              nameRemove="Удалить персонажа"
          ...
          <Row>
            ...
            <Col md={8}>
              ...
              <InfoBlockInline
                icon={sexIcon}
                title="Пол"
                value={sexToString(this.state.character.sex)}
              />
              <InfoBlockInline
                icon={ageIcon}
                title="Возраст"
              ...
              <InfoBlockInline
                icon={nationIcon}
                title="Нация"
                value={this.state.character.nation}
              />
              <InfoBlockInline
                icon={territoryIcon}
                title="Места"
                value={this.state.character.territory}
              />
              <InfoBlockInline
                icon={classIcon}
                title="Класс"
                value={this.state.character.className}
              />
              <InfoBlockInline
                icon={occupationIcon}
                title="Род занятий"
          ...
          <InfoBlock title="Внешность и характер">
            {this.state.character.description}
          </InfoBlock>
          <InfoBlock title="История персонажа">
            {this.state.character.history}
          </InfoBlock>
          <InfoBlock title="Дополнительные сведения">
            {this.state.character.more}
          </InfoBlock>
          <Card
            title="Друзья и знакомые"
            href="/material/character/"
            list={this.state.character.friends}
          />
          <List
            title="Гильдии"
            href="/material/guild/"
            list={this.state.character.guilds}
          />
          <List
            title="Сюжеты"
            href="/material/story/"
            list={this.state.character.stores}
          />
          <List
            title="Отчеты"
            href="/material/report/"
            list={this.state.character.reports}
          />
          ...
          <About
            author={this.state.character.userNickname}
            coauthors={this.state.character.coauthors}
